...
public class Client {
  ...
  private void addDataToSendBuffer(byte[] message, byte commandType, int index, byte[] clientId)
  ...
  private void sendFile(String command, String filePath, String clientId) throws InterruptedException, IOException {
    ...
    addDataToSendBuffer(ByteBuffer.allocate(4).putInt(file.length).array(), (byte) 5, 2, btsId);
    addDataToSendBuffer(file, (byte) 4, 3, btsId);
  }

  private int prepareDownload(String command, String filePath, String clientId)
      throws InterruptedException, IOException {
    addDataToSendBuffer(command.getBytes(), (byte) 7, 1, clientId.getBytes());
...
//            count--;
  ...
  private void execute(String message, byte type, String clientId) throws InterruptedException {
    addDataToSendBuffer(message.getBytes(), type, 1, clientId.getBytes());
    switch (type) {
      case 1:
        currentCommand = "echo";
        break;
    ...
    while (!currentCommand.equals("exit")) {
      ...
      switch (words[0].toLowerCase()) {
        case "echo":
          ...
          clearBuffer();
          execute(message, (byte) 1, "client0001");
          currentCommand = "";
          break;
        case "time":
          currentCommand = words[0].toLowerCase();
          clearBuffer();
          execute(message, (byte) 2, "client0001");
          currentCommand = "";
          break;
        case "upload":
          currentCommand = words[0].toLowerCase();
          clearBuffer();
          sendFile(message, WORK_DIRECTORY_PATH + words[1], "client0001");
          System.out.println("Response from server: " + read());
          currentCommand = "";
          break;
        case "download":
          currentCommand = words[0].toLowerCase();
          clearBuffer();
          prepareDownload(message, WORK_DIRECTORY_PATH + words[1], "client0001");
          System.out.println("Response from server: " + read());
          currentCommand = "";
          break;
        case "exit":
          currentCommand = words[0].toLowerCase();
          clearBuffer();
          execute(message, (byte) 3, "client0001");
          System.exit(0);
          currentCommand = "";
          break;
      }
